{
    "execute_command": "export PROXY_ON='{{user `proxy_on`}}';" + 
       ...
       "export SSH_USERNAME='{{user `ssh_username`}}';"         +
       "export SSH_PASSWORD='{{user `ssh_password`}}';"         +
       "echo {{user `ssh_password`}} | sudo -E -S sh '{{ .Path }}'",
    "scripts": [
